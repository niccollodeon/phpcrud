...
class database {
    ...
    function signup($firstname, $lastname, $sex, $birthday, $username, $password) {
        $con = $this->opencon();
        ...
        if ($existingUser) {
            ...
            return $con->prepare("INSERT INTO users (user_firstname, user_lastname, user_sex, user_birthday, user_name, user_pass) VALUES (?,?,?,?,?,?)")->execute([$firstname, $lastname, $sex, $birthday, $username, $password]);
        }
    }
   ...
   function viewdata($id){
    try{
        $con = $this->opencon();
        $query = $con->prepare("SELECT users.user_id, users.user_firstname, users.user_lastname, users.user_birthday, users.user_sex, users.user_name,users.user_profile_picture,  user_address.barangay, user_address.city, user_address.province, user_address.street from users INNER JOIN user_address ON users.user_id = user_address.user_id WHERE users.user_id = ?");
        $query->execute([$id]);
        return $query->fetch();
    } catch (PDOException $e) {
        return[];
